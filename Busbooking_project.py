...
class Project:
    def __init__(self):
        root = Tk()
        #root.title('Bus Booking Project')
        
        h, w = root.winfo_screenheight(), root.winfo_screenwidth()
        x=1000
        y=550
        root.geometry('%dx%d+%d+%d' % (x, y, (w-x)/2, (h-y)/2))
        
        #To remove title bar
        root.overrideredirect(True)
        #To place it in centre
        ...
        Label(frame2, text="Name:Yashwardhan Tulsyan", font="arial 12 bold",
              fg='blue').grid(row=1, column=0, pady=40)

        Label(frame2, text="Er : 211B360", font="arial 12 bold",
              fg='blue').grid(row=2, column=0)

        Label(frame2, text="Mobile:6205122344", font="arial 12 bold",
    ...
    def seatbooking(self):
        ...
        Label(frame3, text="To").grid(row=0, column=0, padx=10)
        ...
        Label(frame3, text="From").grid(row=0, column=2, padx=10)
        frm = Entry(frame3, width=20)
        frm.grid(row=0, column=3)

        Label(frame3, text="Journey Date (yyyy-mm-dd)").grid(row=0, column=4, padx=10)
        ...
        def showbus():
            ...
            def proceed_to_book():
                ...
                if bus_select.get() == 0:
                    showwarning('Bus Not Selected','Select a bus to proceed...')
                    ...
                    def confirm():
                        ...
                        if nm1 and gender1 and seats1 and mob1 and age1:
                            
                            seats1=int(seats1)
                            age1=int(age1)

                            if seats1 > available[int(bus_select.get())-1]:
                                showwarning('Wrong Entry!','Seats not available!')
                            
                            elif len(mob1) != 10:
                                showwarning('Wrong Entry!','Enter 10 digit Mobile no. to continue...')
                                
                            elif age1>120:
                                showwarning('Invalid Age!','Enter Age less than 120...')
                                age.delete(0, END)
                                ...
                                if choice==True:
                                    ...
                                    #updating seatavail
                                    database.update_seatavail(connection, seats1, bid, date1)
                            ...
                            showwarning('Empty','Fill all the entry')
                            nm.delete(0,END)
                            seats.delete(0,END)
                            mob.delete(0,END)
                            age.delete(0,END)
                    ...
                    bookbtn = Button(frame5, text="Book Seat", command=confirm,
                                     bg='green2')
                    bookbtn.grid(row=1, column=10, padx=10)
                    bookbtn.bind('<Return>',book)
            ...
            if to1 and frm1 and date1:

                bus_list = database.find_bus(connection, to1, frm1, date1)

                if not bus_list:
                    showwarning('No Bus!','No bus found')
                    ...
                    for i in bus_list:
                        ...
                        b = Radiobutton(frame4, text='Bus'+str(a+1),
                               variable=bus_select, value=a+1, indicator=0 ,
                                        bg='green2').grid(row=a+1, column=0, padx=10)
                        ...
                        available.append(i[3])
                        fare.append(i[5])
                        a+=1
                    def proceed(event):
                        proceed_to_book()
                    
                    proceedbtn = Button(frame4, text="Proceed to Book", command=proceed_to_book,
                                        bg='green2')
                    proceedbtn.grid(row=1, column=5, padx=125, pady=20)
                    proceedbtn.bind('<Return>',proceed)
        ...
        showbtn = Button(frame3, text='Show Bus', command=showbus, fg='black',
                    bg='light green')
        showbtn.grid(row=0, column=6, padx=10)
        showbtn.bind('<Return>',enter)
        ...
        homebtn = Button(frame3, fg='black', bg='light green', command=home,
               image=img1)
        homebtn.grid(row=0, column=7)
        homebtn.bind('<Return>',back)

        
        def close():
            
            check = askyesnocancel('Closing','''For Closing Click: Yes
            ...
            if len(mob1) != 10:
                ...
                if (data):
                    ...
                    if c==True:
                        root.destroy()
                        self.seatbooking()
        def checkbooking(event):
            check()
        checkbtn = Button(frame3, text='Check Booking', command=check)
        checkbtn.grid(row=0, column=2, padx=10)
        checkbtn.bind('<Return>',checkbooking)
        connection.commit()
...
Goto Main Page Click: No''')
    ...
    def addbusdetails(self):
        root = Tk()
        root.title('Add Bus Details Menu')
        h, w = root.winfo_screenheight(), root.winfo_screenwidth()
        root.geometry('%dx%d+0+0' % (w, h))
...
Goto Main Page Click: No''')
    ...
    def addoperator(self):
        ...
        def add():
            if not (op.get() and nm.get() and ad.get() and ph.get() and mail.get()):
                showwarning('Empty','Fill all the entry')
                op.delete(0,END)
                nm.delete(0,END)
                ad.delete(0,END)
    ...
    def addbus(self):
        ...
        def add():
            if not(bid.get() and capacity.get() and fare.get() and opid.get() and rid.get()):
                showwarning('Empty','Fill all the entry')
                bid.delete(0,END)
    ...
    def addroute(self):
        ...
        def add():
            if not(rid.get() and nm.get() and sid.get()):
                showwarning('Empty','Fill all the entry')
                rid.delete(0,END)
                nm.delete(0,END)
                sid.delete(0,END)
                ...
                print_details()
                check = database.check_route(connection, int(rid.get()), int(sid.get()))
                if (check):
    ...
    def addrun(self):
        ...
        def add():
            if not(bid.get() and date.get() and seatavail.get()):
                showwarning('Empty','Fill all the entry')
                bid.delete(0,END)
                date.delete(0,END)
                seatavail.delete(0,END)
                
            else:
                capacity = database.check_capacity(connection, int(bid.get()))[0]
                print(capacity)
                if capacity < int(seatavail.get()):
                    showwarning('Seat Not Available','Bus Capacity: '+str(capacity))
                    seatavail.delete(0, END)
                else:
                    print_details()
