...
WHERE MOBILE_NO=? AND BUSID = BID AND B.OPID = O.OPID AND
REF_NO=(SELECT MAX(REF_NO) FROM BOOKING_HISTORY);
'''


UPDATE_SEATAVAIL = '''
UPDATE RUNS
SET SEATAVAIL = SEATAVAIL-? 
WHERE BUSID=? AND RUNSDATE=?;
'''
...
def update_seatavail(connection, seat, bid, travel_date):
    with connection:
        connection.execute(UPDATE_SEATAVAIL, (seat, bid, travel_date))
...
def check_route(connection, routeid, sid):
    with connection:
...
def check_capacity(connection, bid):
    with connection:
        return connection.execute(CHECK_CAPACITY, (bid, )).fetchone()
